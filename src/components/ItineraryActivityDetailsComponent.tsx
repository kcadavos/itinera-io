  ...
  useEffect(()=>{
    const getItineraryDayData =async()=>{
      ...
      if (activityDetailData!=null)
        setActivityDetail(activityDetailData)
    }
  ...
  <>
  {
      ...
      (activityDetail!=null) ? (
          <div className=' flex justify-between '> {/*main div */}
            ...
            <div> 
            {/*icon generator*/}
              <div className={` ${BackgroundColorSwitch(activityDetail.category)} rounded-full w-18 h-18 p-3 border-2 border-black`} >
                <img src={IconSwitch(activityDetail.category)} className="object-cover " alt="Activity Type Icon"/>
          
              </div>
              { !(index===iListLength-1) && (
              <div className='ms-9 w-0.75 h-full bg-black'>
              ...
              <p className={`ps-5 text-2xl font-semibold ${TextColorSwitch(activityDetail.category)} `}> {activityDetail.activity}</p>
            
            
              <p className={`ps-5 text-xl ${TextColorSwitch(activityDetail.category)}`}>{activityDetail.address}</p>
              <p className={`ps-5 text-medium ${TextColorSwitch(activityDetail.category)}`}>{activityDetail.details}</p>
